...
with open(Voter_info) as cvs_file:
    ...
    print("-----------------------------")
    print(f"{CandList[0]}: {percentage0 * 100}% ({vote0})")
    print(f"{CandList[1]}: {percentage1 * 100}% ({vote1})")
    print(f"{CandList[2]}: {percentage2 * 100}% ({vote2})")
    print(f"{CandList[3]}: {percentage3 * 100}% ({vote3})")
    print("-----------------------------")
    ...
    sendOut.write("\n""Election Results")
    sendOut.write("\n""-----------------------------")
    sendOut.write(f"\n""Total Votes: {vtrCount}")
    sendOut.write("\n""-----------------------------")
    sendOut.write(f"\n""{CandList[0]}: {percentage0 * 100}% ({vote0})")
    sendOut.write(f"\n""{CandList[1]}: {percentage1 * 100}% ({vote1})")
    sendOut.write(f"\n""{CandList[2]}: {percentage2 * 100}% ({vote2})")
    sendOut.write(f"\n""{CandList[3]}: {percentage3 * 100}% ({vote3})")
    sendOut.write("\n""-----------------------------")
    sendOut.write(f"\n""Winner: {CandList[0]}")
    sendOut.write("\n""-----------------------------")
